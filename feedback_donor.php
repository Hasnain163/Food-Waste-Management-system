...
$donor_query = $conn->query("SELECT name FROM users WHERE user_id = '$donor_id' AND user_type = 'Donor'");
$donor_data = $donor_query->fetch_assoc();
$donor_name = $donor_data ? $donor_data['name'] : 'Donor User';
...
if ($editing_id !== null) {
    $edit_query = $conn->query("SELECT * FROM feedback WHERE feedback_id = '$editing_id' AND user_id = '$donor_id'");
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM feedback WHERE feedback_id = '$del_id' AND user_id = '$donor_id'");
    header("Location: feedback_donor.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_feedback'])) {
    ...
    if (strlen($comments) < 3) {
        ...
        $conn->query("UPDATE feedback SET comments = '$comments' WHERE feedback_id = '$feedback_id' AND user_id = '$donor_id'");
        header("Location: feedback_donor.php");
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_feedback'])) {
    ...
    if (stripos($comments, 'fuck') !== false || strlen($comments) < 3) {
        ...
        $conn->query("INSERT INTO feedback (user_id, date, comments) VALUES ('$donor_id', CURDATE(), '$comments')");
...
<div class="sidebar">
    <ul>
        <li><a href="dashboard_donor.php"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
        <li><a href="donor_donations.php"><i class="fas fa-gift"></i> Donate Food</a></li>
        <li><a href="my_donations.php"><i class="fas fa-history"></i> My Donations</a></li>
        <li><a href="my_schedule.php"><i class="fas fa-calendar"></i> Pickup Schedule</a></li>
        <li><a href="alerts.php"><i class="fas fa-bell"></i> Alerts</a></li>
        <li class="active"><a href="feedback_donor.php"><i class="fas fa-comment"></i> Feedback</a></li>
    ...
    <div id="my-feedback" class="tab-content <?= $active_tab === 'my-feedback' ? 'active' : '' ?>">
        ...
        <?php if ($edit_feedback_data): ?>
            <form method="POST" class="feedback-form">
                ...
                <a href="feedback_donor.php" class="btn-submit" style="margin-left: 10px;">Cancel</a>
            </form>
        <?php endif; ?>
        ...
        <?php while ($feedback = $my_feedback_result->fetch_assoc()): ?>
            ...
            <div class="feedback-container">
                ...
                <div class="feedback-actions">
                    <a href="feedback_donor.php?edit=<?= $feedback['feedback_id'] ?>" class="btn-submit btn-edit">Edit</a>
                    <a href="feedback_donor.php?delete=<?= $feedback['feedback_id'] ?>" class="btn-submit btn-delete" onclick="return confirm('Delete this feedback?');">Delete</a>
                </div>
            </div>
        <?php endwhile; ?>
    </div>
